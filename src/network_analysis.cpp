...
NetworkAnalysis::NetworkAnalysis(const std::string &filename_v, char sep_v,
                                 const string &filename_e, char sep_e,
                                 bool from_0) {
...
void NetworkAnalysis::read_edges(const std::string &filename, char sep,
                                 bool from_0) {
  std::ifstream file(filename);
  if (!file.is_open()) {
    std::cerr << "Could not open file: " << filename << std::endl;
...
void NetworkAnalysis::print_max_voids_diameter(unsigned nmax) const {
  ...
  for (unsigned i = 0; i < n; ++i)
    std::cout << "(" << voids_pers_int[i].first << ", "
              << voids_pers_int[i].second << ") -> " << max_voids_diameter[i]
              << std::endl;
}

void NetworkAnalysis::compute_network_homology() {
  // By default, since the complex has dimension 1, only 0-dimensional homology
  // would be computed. Here we also want persistent homology to be computed for
  // the maximal dimension in the complex (persistence_dim_max = true)
...
void NetworkAnalysis::compute_alpha_persistent_homology(bool write_file) {
  ...
  voids_pers_int =
      pcoh.intervals_in_dimension(alpha_simplex_tree.dimension() - 1);
  // sort the persistence intervals by decreasing life time
  std::sort(begin(voids_pers_int), end(voids_pers_int),
  ...
  // store diameters
  std::transform(cbegin(voids_pers_int), cend(voids_pers_int),
...
void print_range_simplices(Simplex_tree &st, unsigned start_index,
  ...
  std::clog << "Iterator on simplices from " << start_index << " to "
