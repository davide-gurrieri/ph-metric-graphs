...
int main(int argc, char *argv[]) {
  ...
  NetworkAnalysis net =
      NetworkAnalysis(vertices_file_name, vertex_sep, edges_file_name, edge_sep,
                      indexes_from_0);

  std::cout << std::endl;
  std::cout << "####################################" << std::endl;
  std::cout << "First 5 simplices of the network simplicial complex:"
            << std::endl;
  ...
  chrono.stop();
  std::cout << "Elapsed time for network homology computation: "
            << chrono.wallTime() << " microsec" << std::endl;

  chrono.start();
  net.compute_alpha_persistent_homology(false);
  chrono.stop();
  std::cout << "Elapsed time for alpha persistent homology computation: "
            << chrono.wallTime() << " microsec" << std::endl;
