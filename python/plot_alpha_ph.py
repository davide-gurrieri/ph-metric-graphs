...
for i in range(dim): # 0 1 ... dim-1
    ...
    gd.plot_persistence_barcode(barcode_i, max_intervals=100, colormap=col, alpha=1)
    plt.savefig(f'../output/plot/barcode_dim_{i}.pdf')

gd.plot_persistence_diagram(alpha_barcode,alpha=1,legend=True)
plt.savefig(f'../output/plot/persistence_diagram.pdf')
